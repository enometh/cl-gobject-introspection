...
(defclass gobject-subclassable-info ()
  ((gir-parent :initarg :gir-parent)	;object-class
   ...
   ;; the vfunc - `get-vfunc-desc'. we then generate defcallback forms
...
;;;
;;;
(defun %generate-vfunc-defcallback-form (obj vfunc-name)
  "Emit forms for a lisp defmethod and a corresponding cffi:callback
which calls a lisp function."
  (let* ((callback-name (%gobject-subclassable-get-name obj :vfunc vfunc-name))
	 (lisp-function-name (%gobject-subclassable-get-name obj :vfunc
							     (concatenate 'string vfunc-name "_lisp")))
	 (vfunc-info
	  (with-slots ((parent-obj gir-parent)) obj
	    (object-class-find-vfunc-info parent-obj vfunc-name))))
    (generate-cffi-defcallback vfunc-info callback-name lisp-function-name)))

(defun %gobject-subclassable-define-vfunc-defcallbacks (obj)
  (with-slots (vfuncs) obj
    (when vfuncs
      `(progn ,@(mapcar (lambda (vfunc)  (%generate-vfunc-defcallback-form obj vfunc))
		       vfuncs)))))
...
(defun %generate-class-init-override (obj vfunc-name)
  (let ((klass (%gobject-subclassable-get-name obj :class-arg))
	...
	(callback-name (%gobject-subclassable-get-name obj :vfunc vfunc-name)))
