...
(defun proxy-call-sync (dbus-proxy method-name args &key (timeout-msec 15000) cancellable (call-flags (nget *gio* "DBusProxyFlags" :none)))
...
(defun generate-defun-proxy-call-sync (dbus-proxy method-name name)
  (multiple-value-bind (in-args-names out-args-names)
	...
	`(defun ,name (dbus-proxy-object ,@param-names
		      ...
		      (call-flags (nget *gio* "DBusProxyFlags" :none)))
